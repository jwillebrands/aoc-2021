...
fun parseInput(lines: List<String>): Pair<Set<Point>, List<Fold>> {
    ...
    val points = lines.subList(0, emptyLine).map { it.split(",", limit = 2) }.map { (x, y) -> Point(x.toInt(), y.toInt()) }.toSet()
    val folds = lines.subList(emptyLine + 1, lines.size).map { it.split("=", limit = 2) }
        .map { (axis, pos) -> Fold(if (axis.endsWith("x")) FoldAxis.HORIZONTAL else FoldAxis.VERTICAL, pos.toInt()) }
...
fun paint(points: Set<Point>) {
    ...
    for (y in 0 .. maxY) {
        println((0 .. maxX).joinToString(separator = ""){ x -> if (points.contains(Point(x, y))) "#" else " "})
    }
}
