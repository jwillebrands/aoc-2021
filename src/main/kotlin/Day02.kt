...
fun executeCommands(lines: List<String>, commandFactory: CommandFactory): Submarine {
  return lines.map{it.split(" ")}
    	.fold(Submarine()) {sub, cmd -> commandFactory(cmd[0])(sub, cmd[1].toInt())}
}

fun solvePart1(lines: List<String>): Int {
    val commandFactory: CommandFactory = {command -> when(command) {
...
fun solvePart2(lines: List<String>): Int {
  val commandFactory: CommandFactory = {command -> when(command) {
    "forward" -> {sub, amount -> sub.copy(hpos = sub.hpos + amount, depth = sub.depth + sub.aim * amount)}
